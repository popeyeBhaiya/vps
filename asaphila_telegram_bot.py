...
# Built-in proxy (US residential). Used when user has not set one via /proxy.
# Format for checker: host:port:user:pass
BUILTIN_PROXY = "geo.g-w.info:10080:user-wwPtmya6KGr9VcaC-type-residential-country-US:lIapty7sbISvLJw5"
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "Built-in US residential proxy is used by default.\n"
        "/check — how to send cards\n"
        "/proxy [proxy] — set your proxy (or send /proxy to clear and use built-in)\n\n"
...
async def proxy_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if proxy:
        ...
        await update.message.reply_text("Proxy cleared. Using built-in proxy.")


def get_user_proxy(context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
    """User proxy if set, else built-in proxy."""
    if context.user_data and "proxy" in context.user_data:
        p = context.user_data["proxy"]
        if p:
            return p
    ...
    try:
        for idx, card in enumerate(cards, 1):
            ...
            if s == "CHARGED":
                ...
                err += 1
            await update.message.reply_text(format_result(idx, total, r))
    except Exception as e:
        await status_msg.edit_text(f"Error: {e}")
        return

    await update.message.reply_text(
        f"CHARGED: {charged} | LIVE: {live} | DEAD: {dead} | ERR: {err}"
    )
